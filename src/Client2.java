...
public class Client2 {
    ...
    void connectToServer() {
        ...
        new Thread(() -> {
            try {
                ...
                if (resp == null || resp.startsWith("ERROR:")) {
                    SwingUtilities.invokeLater(() -> loginPanel.setStatus(
                            resp != null && resp.contains("TAKEN")
                                    ? "Nom déjà utilisé !" : "Connexion échouée !", true));
                    return;
                }
                ...
                // Démarrer les listeners AVANT les requêtes
                ...
                SwingUtilities.invokeLater(() ->
                        loginPanel.setStatus(
                                "Impossible de joindre le serveur (HOST=" + HOST + ")", true));
            }
        }).start();
    }
    ...
    void startControlListener() {
        new Thread(() -> {
            try {
                ...
                SwingUtilities.invokeLater(() ->
                        chatPanel.addSystemMessage(" Connexion perdue au serveur"));
            }
    ...
    void handleControlEvent(String line) {
        ...
        if (line.startsWith("ADDED_TO_GROUP:")) {
            ...
            SwingUtilities.invokeLater(() ->
                    JOptionPane.showMessageDialog(frame,
                            "you avez été ajouté à un groupe !",
        ...
        if (line.startsWith("MEMBER_ADDED:")) {
            ...
            if (p.length == 2)
                SwingUtilities.invokeLater(() ->
                        JOptionPane.showMessageDialog(frame,
                                p[1] + " a été ajouté au groupe.",
                                "Membre ajouté", JOptionPane.INFORMATION_MESSAGE));
            return;
        }
        if (line.startsWith("HISTORY_START:")) { receiveHistory(line.substring(14)); return; }
        if (line.startsWith("ERROR:"))
            SwingUtilities.invokeLater(() ->
                    JOptionPane.showMessageDialog(frame, "Erreur : " + line.substring(6),
    ...
    void startMessageListener() {
        new Thread(() -> {
            try {
                ...
                System.out.println("[CLIENT] Canal message fermé");
            }
    ...
    void handleIncomingMessage(String line) {
        ...
        // ── Message VOCAL privé ───────────────────────────────────────────
        // VOICE_PRIVATE:<from>:<durationSec>:<base64_pcm>
        ...
        // ── Message VOCAL groupe ──────────────────────────────────────────
        // VOICE_GROUP:<groupId>:<groupName>:<from>:<durationSec>:<base64_pcm>
    ...
    void sendMessage() {
        ...
        // addMessage est appelé depuis l'EDT (ActionListener) — pas besoin de invokeLater
    ...
    void startVoiceMessage() {
        ...
        try {
            ...
            if (!AudioSystem.isLineSupported(info)) {
                SwingUtilities.invokeLater(() ->
                        JOptionPane.showMessageDialog(frame,
                                ...
                                "Erreur audio", JOptionPane.ERROR_MESSAGE));
                return;
            }
            ...
            chatPanel.addSystemMessage(" Enregistrement en cours…");
            ...
            SwingUtilities.invokeLater(() ->
                    JOptionPane.showMessageDialog(frame,
                            "Impossible d'ouvrir le microphone : " + ex.getMessage(),
                            "Erreur", JOptionPane.ERROR_MESSAGE));
        }
    }
    ...
    void stopAndSendVoiceMessage() {
        ...
        if (durationSec < 1) durationSec = 1; // minimum 1 sec

        // Feedback visuel
        chatPanel.setRecording(false);

        if (audioData.length == 0) {
            chatPanel.addSystemMessage(" Message vocal vide, annulé.");
            return;
        }

        // Encoder en Base64 pour transport sur le canal texte
        String encoded = Base64.getEncoder().encodeToString(audioData);

        // Envoyer selon le mode (privé ou groupe)
        if (currentChatIsGroup) {
            // VOICE_MSG_GROUP:<from>:<groupId>:<durationSec>:<base64>
            messageOut.println("VOICE_MSG_GROUP:" + username + ":"
                    + currentChatTarget + ":" + durationSec + ":" + encoded);
        } else {
            // VOICE_MSG_PRIVATE:<from>:<to>:<durationSec>:<base64>
            messageOut.println("VOICE_MSG_PRIVATE:" + username + ":"
                    + currentChatTarget + ":" + durationSec + ":" + encoded);
        }

        // Afficher notre propre bulle vocale
    ...
    // ══════════════════════════════════════════════════════════════════════
    //  APPEL VOCAL TEMPS RÉEL (inchangé)
    // ══════════════════════════════════════════════════════════════════════
    void toggleCall() {
        if (!inCall) startCall();
        else         endCall();
    ...
    void startCall() {
        ...
        new Thread(() -> {
            try {
                ...
                SwingUtilities.invokeLater(() ->
                        JOptionPane.showMessageDialog(frame,
                                "Impossible de démarrer l'appel : " + ex.getMessage(),
                                "Erreur audio", JOptionPane.ERROR_MESSAGE));
            }
        }).start();
    }
    ...
    void showCreateGroupDialog() {
        ...
        JDialog dialog = new JDialog(frame, "Nouveau groupe", true);
        ...
        JLabel lbl = new JLabel("Nom du groupe :");
    ...
    void showAddMemberDialog(String gid) {
        ...
        JDialog dialog = new JDialog(frame, "Ajouter un membre", true);
        dialog.setSize(340, 350);
        dialog.setLocationRelativeTo(frame);
        dialog.getContentPane().setBackground(AppColors.BG_SIDEBAR);
        dialog.setLayout(new BorderLayout(0, 0));

        JLabel title = new JLabel("Sélectionner un utilisateur :");
        ...
        for (Map.Entry<String, Boolean> e : userOnline.entrySet()) {
            ...
            JRadioButton rb = new JRadioButton(u + (online ? "" : " (hors ligne)"));
    ...
    void disconnectFromServer() {
        endCall();
        // Arrêter aussi l'enregistrement en cours si nécessaire
        if (vmMic != null) {
            if (vmRecordThread != null) { vmRecordThread.interrupt(); vmRecordThread = null; }
        ...
        SwingUtilities.invokeLater(() -> { loginPanel.setStatus("Déconnecté", false); showLogin(); });
    }
}